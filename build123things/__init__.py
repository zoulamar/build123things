...
class Thing (ABC, metaclass=ThingMeta):
    ...
    def matrix_of_inertia(self, precise=False, tidy = True) -> Tuple[np.ndarray, np.ndarray]:
        ...
        return: matrix_of_inertia, center_of_mass w.r.t. Thing's origin. """
        ...
        #else:
        #    what:bd.Part = res.scale(1e-3) # Millimeters (Design) to Meters (SI)
        #    filename = random_tmp_fname(ext=".stl")
        #    bd.export_stl(what, filename, angular_tolerance=.01 if precise else 0.1)
        #    _, cog, inertia = pystl.PySTL.mesh.Mesh.from_file(filename).get_mass_properties()
        ...
        if res is None:
            ...
            what:bd.Part = res.scale(1e-3) # Millimeters (Design) to Meters (SI)
            return np.array(what.matrix_of_inertia), np.array(what.center(center_of=bd.CenterOf.MASS).to_tuple())
