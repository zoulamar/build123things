"""
Implements exporting a Thing to MuJoCo file format, but with the possibility to select the kinematic root arbitrarily.
"""
...
def fmt_loc (x:bd.Location, scale:float=1) -> dict[str,str]:
    return {
    ...
    for traversor_i, traversor in enumerate(thing.walk()):
        ...
        if traversor.joint is not None and traversor.parent is not None:
            if isinstance(traversor.joint, Rigid):
                ...
                t2 = bd.Location((0,0,0),(180,0,90)) * traversor.joint.transform(
        ...
        if mesh_name not in xml_meshes:
            ...
            if res is not None:
                ...
                bd.export_stl(res, str(stl_file))
