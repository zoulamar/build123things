...
""" This module is used to vizualize Things, Joints or reference geometries in CQ-Editor or ocp-vscode. """
...
# Visualization backend import.
try:
    # This should trigger while the script is run from CQ-Editor. Or if user wants to use some custom visualiser.
    from __main__ import show_object
    if "import" in DEBUG: print(f"Using `show_object` found in `__main__` module.")
except ImportError:
    try:
        from ocp_vscode import show_object #type:ignore
        if "import" in DEBUG: print(f"Using `show_object` found in `ocp_vscode` module. By default, open http://127.0.0.1:3939/viewer in a browser of your choice and run `python3 -m ocp_vscode` in a terminal.")
    except ImportError:
        if "import" in DEBUG: print(f"Providing a dummy `show` method as no other was found.")
    ...
    print("Dark magic happened.")

@singledispatch
def show(x:Any, *args, **kwargs) -> None:
...
@show.register
def _ (thing:Thing, recurse=False) -> None:
    """ Either shows a Thing together with subassemblies or a thing with reference geometries. """
...
def _ (thing_grounder:TransformResolver):
    ...
    if res is not None:
        ...
        show_object(res, name=thing_grounder.codename(), options={"color":thing_grounder.__material__.color.rgba, "alpha":1}) # type: ignore
    else:
